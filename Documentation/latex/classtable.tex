...
Magic method for setting or getting related tables. If function is called without argument, it will find the related tables. It will automatically save the connectin! \begin{Desc}
\item[\hyperlink{todo__todo000006}{Todo}]If function is called with a related table as an argument, it will be connected to it. \end{Desc}
...
\end{DoxyParams}
\begin{Desc}
\item[\hyperlink{todo__todo000007}{Todo}]\$args could be a table of \$name \end{Desc}
...
\end{DoxyParams}
\begin{Desc}
\item[\hyperlink{todo__todo000008}{Todo}]find a way to reduce arguments \end{Desc}
\begin{DoxyReturn}{Returns}
\$this 
\end{DoxyReturn}
...
Returns the number of rows that result from a SELECT query. Parameters are the same as for \hyperlink{classtable_a54c67ef7f9e768176aedf92e3856abaa}{find()}, except that you can't return only some columns. Also, ordering is not included in the query. 
\begin{DoxyParams}[1]{Parameters}
...
Delete the corresponding record from the database \hypertarget{classtable_a54c67ef7f9e768176aedf92e3856abaa}{
...
\hline
bool & {\em \$import} & -\/ whether to import the resulting stuff into tables or not. Default is true. Useful if you use the \$columns parameter to retrieve values from other tables. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
array 
...
Static wrapper for \hyperlink{classtable_a54c67ef7f9e768176aedf92e3856abaa}{find()}. Arguments the same. 
\begin{DoxyParams}[1]{Parameters}
string & {\em \$table} & Table in which to search \\
\hline
array & {\em \$filters} & Filters to apply. Can be name=$>$value pair, SQL statement, or recursive array relating to conditions on other tables \\
